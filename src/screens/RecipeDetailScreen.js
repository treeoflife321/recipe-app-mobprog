    ...
    <View className="flex-1 bg-white relative">
        ...
        <Animated.View entering={FadeIn.delay(200).duration(1000)} className="w-full absolute flex-row justify-between items-center pt-14">
            <TouchableOpacity onPress={()=> navigation.goBack()} className="p-2 rounded-full ml-5 bg-white">
                <ChevronLeftIcon size={hp(3.5)} strokeWidth={4.5} color="blue" />
            </TouchableOpacity>
                                    ...
                                    return (
                                        <View key={i} className="flex-row space-x-4">
                                            ...
                                            <View className="flex-row space-x-2">
                                                    <Text style={{fontSize: hp(2)}} className="font-extrabold text-neutral-700">{meal['strMeasure'+i]}</Text>
                                    ...
                                    {/* YoutubeIfram uses webview and it does not work properly on android (until its fixed we'll just show the video on ios) */}
                                    {
                                        ios? (
                                            ...
                                            <TouchableOpacity className="mb-5" onPress={()=> handleOpenLink(meal.strYoutube)}>
                                                <Text className="text-red-600" style={{fontSize: hp(2)}}>{meal.strYoutube}</Text>
                                            </TouchableOpacity>
